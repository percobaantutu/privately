    ...
    <div className={`flex justify-between items-center border-b py-2 mb-5 ${isTeacherView ? "px-4 md:px-8" : "px-4 md:px-0"}`}>
      ...
      {!user || user.role !== "teacher" ? (
        <ul className="hidden lg:flex justify-between items-center gap-4 text-md font-semibold">
          ...
          <NavLink to="/teachers">
            <li className="py-1">{t("navbar.allTeachers")}</li>
          </NavLink>
          <NavLink to="/about">
            <li className="py-1">{t("navbar.aboutUs")}</li>
          </NavLink>
          <NavLink to="/contact">
          ...
          <>
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent className="w-56">
                <DropdownMenuGroup>
                  <DropdownMenuItem onClick={() => navigate("/my-profile")}>{t("navbar.myProfile")}</DropdownMenuItem>
                  {user.role === "teacher" ? (
                    <DropdownMenuItem onClick={() => navigate("/teacher/dashboard")}>{t("navbar.teacherDashboard")}</DropdownMenuItem>
                  ) : (
                    <>
                      <DropdownMenuItem onClick={() => navigate("/my-appointments")}>{t("navbar.mySessions")}</DropdownMenuItem>
                      <DropdownMenuItem onClick={() => navigate("/my-disputes")}>{t("navbar.myDisputes")}</DropdownMenuItem>
                    </>
                  )}
                </DropdownMenuGroup>
              ...
              {!user || user.role !== "teacher" ? (
                <ul className="flex flex-col gap-2 text-lg font-medium">
                  <NavLink to="/" onClick={() => setShowMenu(false)} className={({ isActive }) => (isActive ? "text-primary font-bold" : "")}>
                    <li className="px-2 py-2 hover:bg-gray-100 rounded-lg">{t("navbar.home")}</li>
                  </NavLink>
                  <NavLink to="/teachers" onClick={() => setShowMenu(false)} className={({ isActive }) => (isActive ? "text-primary font-bold" : "")}>
                    <li className="px-2 py-2 hover:bg-gray-100 rounded-lg">{t("navbar.allTeachers")}</li>
                  </NavLink>
                  <NavLink to="/about" onClick={() => setShowMenu(false)} className={({ isActive }) => (isActive ? "text-primary font-bold" : "")}>
                    <li className="px-2 py-2 hover:bg-gray-100 rounded-lg">{t("navbar.aboutUs")}</li>
                  </NavLink>
                  <NavLink to="/contact" onClick={() => setShowMenu(false)} className={({ isActive }) => (isActive ? "text-primary font-bold" : "")}>
                    <li className="px-2 py-2 hover:bg-gray-100 rounded-lg">{t("navbar.contact")}</li>
                  </NavLink>
